...
---
# 🏗️ Architectural Design Document: Graduated System CPRU
...
- **States:** `PENDING` $\rightarrow$ `UNDER_REVIEW` $\rightarrow$ `VERIFIED` | `REJECTED`.
...
- **Workflow:** 
    1. Admin triggers export $\rightarrow$ Job dispatched to queue $\rightarrow$ Immediate "Processing" response.
...
- **Silk Pink Rarity Rule:** Primary accent color (`#e91e63`) used on $\le 10\%$ of the UI to maintain focus.
